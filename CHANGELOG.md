...
### Refactor: 类型推断判定树重构 + 去重归一与 isCSV 解耦（含一个数据丢失修复）

本轮的动因是一次独立审查暴露的**系统性根因**：`detectScalar` / `inferType` 是一组「谁先命中谁赢」的正则启发式，规则互相抢跑、每条都过宽 —— 前两轮每修一个反例就会冒出新的绕过形态。本次改为**显式判定树**，并顺带修掉一个**由上一版本引入的数据丢失缺陷**。

**① 【数据丢失 · 最高优先级】结构化数值列被判 number 但写入空值（v1.3.3 引入）**
- **现象**：`1,000` / `$3.5` / `¥1,200.50` / `-1,234.5` 这类值被判为 `number`（类型判定正确），但归一与写入得到空值 —— `canonRawCell("1,000","number")` 返回 `""`、`buildCell` 产出 `{content:0, isNotEmpty:false}`，**整列金额被静默写空**。这比 v1.3.2 的旧行为（误判为多选，至少数据仍在）更严重。
- **根因**：v1.3.3 引入的识别函数 `isExplicitNumber` 认得千分位/货币形态，但归一（`normalizeNumberString`）与写入（`buildCell`）仍只认 `Number(x)` —— 三者不同源，而 `Number("1,000")` 是 `NaN`。
- **修复**：抽出统一的 `parseStructuredNumber()`（去首尾货币符号与空白 → 排除 `0x`/`0b`/`0o` 与 `_` → 标准千分位 → 十进制/科学计数），使**识别、归一、写入三处同源**，源侧与目标侧严格对称。

**② 判定树重构：`detectScalar` 从「并列抢跑」改为「显式顺序」**
- 新顺序（自上而下，命中即返回）：空 → `text`；`isEmail` → `email`；`isUrl` → `url`；`isPhoneStrict` → `phone`；`isStructuredNumber` → `number`；`isDateStrict` → `date`；`isNumberStrict` → `number`；兜底 `text`。
- 修复结果（下列输入此前**全部**误判）：

| 输入 | 修复前 | 现在 |
|---|---|---|
| `192.168.1.1` | phone | **text** |
| `2024.13.45` | phone | **text** |
| `1721483847000` | phone | **number** |
| `report.pdf` | url | **text** |
| `archive.zip` | url | **text** |
| `Mr.Smith` | url | **text** |
| `0x1f` | number | **text** |
| `data:text/html,x` | mSelect | **text** |

- **URL 规则**：新增 `isUrlStrict` —— 协议 / `www.` / 协议相对 `//` 直通；裸域名要求 TLD 在白名单内，并**先排除常见文件后缀**（`pdf`/`zip`/`txt`/`doc`/…）。保留 `<a href="…">` 的 anchor 提取能力。
- **电话规则**：新增 `isPhoneStrict` —— 日期优先排除、点分四段形态（IPv4 / 非法日期如 `2024.13.45`）排除、数字位数收紧为 7–15 位（无分隔符形态要求 11–12 位）。
- **数值规则**：新增 `isNumberStrict`，排除 JS 特有字面量（`0x`/`0b`/`0o`/`_`）。
- `inferType` 的列级策略同步调整：mSelect 的计数条件改为「含分隔符 **且** `detectScalar(v) === "text"`」，因此不再需要此前那条「前置数值守卫」补丁。

**③ 日期解析收紧：彻底移除 `Date.parse` 兜底**
- **现象**：`parseFlexibleDateToMs` 会给 `1.5`（→ `978624000000`）、`1.2.3`、`www.abc.com/path?q=1`、`$3-5` 这类值算出一个**莫须有的日期**；`2024-02-30` 更是被 V8 滚动成 3 月 1 日而接受。类型判定层挡住了大部分，但**列类型一旦是 date，错误日期就会写进库**。
- **修复**：只接受 10/13 位时间戳与 `YYYY-M-D` / `YYYY/M/D` / `YYYY.M.D` / `YYYY年M月D日` / `YYYY年` / `YYYY-M` 这些严格模式，并做**日/月边界校验**（`okMD`，含闰年 2 月），其余一律 `NaN`。
- **行为变化**：日期值统一到**同一时区语义**（此前 `2024-01-01` 按 UTC 解释、`2024/1/1` 按本地解释，两者相差 8 小时）；现在二者走同一正则分支，结果一致。

**④ B5：跨源类型时的整行去重失效（会导致重复插入）**
- **现象**：CSV 写入的多选 `AI Agent 知识库` 在库中存为 3 个标签；同一份数据改用 Markdown 导入时，源侧把整段当作 1 个标签 → 源 key `ai agent 知识库` ≠ 目标 key `知识库,agent,ai` → 判为新增 → **重复插入**；预览里显示的「重复 N 行」也随之偏小。
- **根因**：`readAV` 用**本次运行的 `isCSV`** 归一**已存**数据，而库中存储形态由**写入时的 `isCSV`** 决定 —— 读端规则随运行变化，**非幂等**。
- **修复**：**把去重归一与 `isCSV` 解耦** —— `canonRawCell` / `canonValueCell` 的 mSelect 分支恒定按最细粒度切分（等价 `isCSV=true`）。写入形态仍由 `isCSV` 决定，**存储格式零变化**；`buildRowKey` / `readAV` / `computeIncremental` / `appendToExisting` 与预览渲染**全部零改动**即自动一致（预览的「重复 N 行」与实际跳过行数保持同步）。
- **已评估的失效方向**：本方案的失效是「漏判新增（假重复）」而非「重复插入」，且触发需**整行所有列逐字相同**、仅 mSelect 的切分语义不同 —— 属少数场景。备选方案均已排除：「读端按写时属性」（内核管理的 AV schema 无稳定的自定义字段可落）、「读端双规则」（实测无法修复本问题，且会把另一个方向从「正确判重」退化成「判新增」）。
- `isCSV` 形参保留（兼容调用方签名），但归一分支不再读取它。

**⑤ select 列 canon 对称**
- **现象**：源侧 `canonRawCell(select)` 只做整串 lowercase（原序、不去重），目标侧走 `normalizeMulti`（排序 + 去重）→ 大小写与顺序双重不对称。
- **修复**：两侧统一为「整值 trim + lowercase 后走 `normalizeMulti`」。**刻意不做逗号切分** —— select 语义上是单值，避免 `"Hong Kong"` 被归一成 `"hong,kong"` 这类过度归一。

**⑥ 测试**
- 用例数 **138 → 236**（20 个文件）。新增 `v140-*` 五组共 98 条，覆盖判定树全表与边界、日期严格解析、数值写入对称、跨源去重端到端、select 对称。
- `test/type-feature-table.test.js` 中 3 条此前标注「已知缺陷 · 转 backlog」的断言（`192.168.1.1` / `report.pdf` / `0x1f`）本轮已修，断言改回原期望值。

**附：已知的设计取舍（有意为之，非缺陷）**
- `archive.zip` 判 `text` —— TLD 白名单**不收录 `zip`**（`.zip` 虽是真实 gTLD，但「是文件名」远多于「是主机名」）；带协议的 `https://x.zip` 仍直通。
- 带单位后缀的值（如 `100kg`）**不支持**，判 `text`（避免把 `100kg` 静默变成 `100` 而丢失单位）。
- 10/13 位 epoch 纯数字判 `number`（类型推断层**不判时间戳**，否则「10 位数值列」会被误判成日期）；date 列的**值**解析仍支持时间戳。
- 11–12 位纯整数判 `phone`（中国手机号形态）；13 位及以上由长度守卫挡回 `number`。
